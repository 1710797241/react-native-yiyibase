...
var NBCompFlatList = /** @class */ (function (_super) {
    ...
    NBCompFlatList.prototype.fetchData = function (fetchParams, page, size, clear) {
        var _this = this;
        if (!this.props.api) {
            nbLog('下拉刷新列表组件', '请设置api属性！');
            return;
        }
        var params = clear !== undefined && clear ? fetchParams : __assign(__assign(__assign({}, this.props.params), this.fetchParams), fetchParams);
        var pnf = this.props.pageNoField ? this.props.pageNoField : 'pageNumber';
        var pns = this.props.pageSizeField ? this.props.pageSizeField : 'pageSize';
        ...
        return this.onRefreshStart(this.page > 1).then(function (is) {
            if (!is)
    ...
    NBCompFlatList.prototype.changeDataList = function (func) {
        var _this = this;
        return new Promise(function (res, rej) {
            try {
                var data_2 = func(_this.state.data);
                _this.setState({
                    data: data_2
                }, function () {
                    res(data_2);
    ...
    NBCompFlatList.prototype.onRefreshStart = function (isLoadMore) {
        ...
        return new Promise(function (res, rej) {
            if (isLoadMore !== undefined && isLoadMore) {
                if (_this.state.data) {
                    if (_this.total > 0 && _this.state.data.length >= _this.total) {
                        res(false);
                        return;
                    }
                }
    ...
    NBCompFlatList.prototype.render = function () {
        ...
        return <View style={this.props.style}>
            ...
            if (_this.state.data.length < _this.total && _this.total > 0 && _this.state.data.length >= _this.size) {
                _this.page = _this.page === undefined || _this.page === NaN ? 1 : _this.page;
                _this.fetchData(_this.fetchParams, _this.page + 1);
            }
        }} onEndReachedThreshold={this.props.onEndReachedThreshold === undefined ? this.props.onEndReachedThreshold : 1} style={[{ flex: 1 }, this.props.flatListProps]}/>
